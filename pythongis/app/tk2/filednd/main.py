...
def _load_tkdnd(master):
    ...
    if sys.platform == "win32":
        ...
        tkdndlib = os.path.join(os.path.split(__file__)[0], "Linux")
    else:
        tkdndlib = None
    # load them
    os.environ['TKDND_LIBRARY'] = tkdndlib  # Not sure if needed
    if tkdndlib:
...
class TkDND(object):
    ...
    _subst_format = ('%A', '%a', '%b', '[join %D {|}]', '%d', '%m', '%T',
            '%W', '%X', '%Y', '%x', '%y')
    ...
    def _dndsubstitute(self, *args):
        ...
        if T == "CF_HDROP": # TODO: ALSO MAKE SURE WORKS ON LINUX AND MAC
            D = D.split("|")
        else:
            D = [D]
