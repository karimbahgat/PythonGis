...
class Geography(object):
    def __init__(self, obj=None, **kwargs):
        ...
        if isinstance(obj, dict):
            ...
            self._geoj = obj.__geo_interface__
        elif kwargs:
            self._geoj = kwargs
        elif isinstance(obj, Geography):
            self._geoj = obj._geoj
        else:
            raise Exception()

    @property
    def __geo_interface__(self):
        return self._geoj.copy()

    @property
    def type(self):
        return self._geoj["type"]

    @property
    def coordinates(self):
        return self._geoj["coordinates"]

    @property
    def geogs(self):
        for geoj in self._geoj["geometries"]:
            yield Geography(geoj)

    @property
    def length(self):
        
        def _handle(geog):
            if "Point" in geog.type:
                return 0.0
     
            elif geog.type == "LineString":
                length = sum((_vincenty_distance(p1,p2)
                              for p1,p2 in _pairwise(geog.coordinates))
                             )
                return length
     
            elif geog.type == "MultiLineString":
                length = sum((_vincenty_distance(p1,p2)
                              for line in geog.coordinates
        ...
        if self.type == "GeometryCollection":
            length = sum((_handle(geog) for geog in self.geogs))
            return length
    ...
    def walk(self, direction, distance):
        if self.type == "Point":
            endpoint = _walk(self.coordinates, direction, distance)
            return Geography(type="Point", coordinates=endpoint)

        else:
            raise NotImplementedError("Walk is only implemented for single point geometries")

    def buffer(self, distance, resolution=100):

        def _handle(geog):
            if "Point" in geog.type:
                incr = 360 / float(resolution)

                def singlebuff(subgeog):
                    point = subgeog.coordinates
                    buffercoords = []
                    cur = 0
                    while cur < 360:
    ...
    def line_to(self, other, great_circle=False, segments=None):
        if not great_circle:
            if self.type == "Point" and other.type == "Point":
                coords = [tuple(self.coordinates), tuple(other.coordinates)]
                return Geography(type="LineString", coordinates=coords)

            else:
                raise NotImplementedError("Only implemented between two single points")

        else:
            if self.type == "Point" and other.type == "Point":
                segments = segments or 100 # nr of vertices for entire line
                coords = _great_circle_path(self.coordinates, other.coordinates, segments=segments)
                return Geography(type="LineString", coordinates=coords)
