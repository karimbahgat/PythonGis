...
polys = pg.vector.data.VectorData("data/ne_10m_admin_1_states_provinces.shp",
                                  encoding="latin",
...
vect = pg.vector.data.VectorData("data/ne_10m_admin_1_states_provinces.shp", encoding="latin")
...
vect = pg.vector.data.VectorData("data/ne_10m_populated_places_simple.shp", encoding="latin")
mapp.add_layer(vect, title="Cities",
                    #fillsize=1, # TODO: legend size should be set to same pixelsize as dist parsed for map
                               #{"breaks":"natural",
                                ...
                                #},
                   outlinewidth=0.2,
                    fillcolor=dict(breaks="unique",colors=dict(Riyadh="red",Casablanca="yellow"), key=lambda f:f["name"]), #"red",
                              #{"breaks":"natural",
                              #  "key":lambda f: float(f["Average_nk"]),
...
mapp.add_layer(vect, title="Cities",
                    #fillsize=1, # TODO: legend size should be set to same pixelsize as dist parsed for map
                               #{"breaks":"natural",
                                ...
                                #},
                   outlinewidth=0.2,
                    fillsize=dict(breaks="proportional", sizes=[0.2, 2.0], key=lambda f: -f["labelrank"]), #"red",
                              #{"breaks":"natural",
                              #  "key":lambda f: float(f["Average_nk"]),
                              #  "symbolvalues":[(0,255,0,255),(255,255,0,255),(255,0,0,255)],
                              # },
                   #fillcolor=dict(breaks="proportional", colors=["green","yellow","red"], key=lambda f: -f["labelrank"]), #"red",
                    sortkey=lambda f: -f["labelrank"],
                    sortorder="incr",
                   #nolegend=True,
                  )
...
vect = pg.vector.data.VectorData("data/ne_10m_populated_places_simple.shp", encoding="latin")
mapp.add_layer(pg.renderer.VectorLayer(vect, title="Population",
                                                fillsize=0.3,
                                                           #{"breaks":"natural",
                                                            #"key":lambda f: float(f["Average_nk"]),
                                                            #"symbolvalues":[0.2, 2.0]
                                                ...
                                                fillcolor=pg.renderer.Color("red", opacity=190),
                                                          ...
                                                          #  "symbolvalues":[(0,255,0,255),(255,255,0,255),(255,0,0,255)],
                                                          # },
                                                sortkey=lambda f: -float(f["labelrank"]),
                                                sortorder="incr", 
                                              ))
...
###
print("\n"+"point test")
vect = pg.vector.data.VectorData("data/ne_10m_populated_places_simple.shp")
print vect.fields

vect.view(1000,500,
            fillsize={"breaks":"headtail",
                        "key":lambda f: float(f["pop"]),
                        "valuestops":[0.2, 2.0]
                      },
...
###
print("\n"+"polygon test")
vect = pg.vector.data.VectorData("data/ne_10m_admin_1_states_provinces.shp",
                                 encoding="latin")
print(vect.fields)
vect.view(1000,500,
            fillcolor={"breaks":"equal",
